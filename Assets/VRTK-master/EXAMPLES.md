move an object in the scene with the laser pointer

have but where: point to something and grab it- but snaps to controller.

the solution would be to (update the grab attach point) to the (pointer tip) and then (limit the length of the pointer to not exceed the length when it was grabbed) at


you'd probably also want the pointer ray to go through the object that is being grabbed otherwise it will always be set to the length of the collision to that object
...
Comes with controller pointer events listener example.

simple pointer
controller events
controller pointer events listener example // see events for when the pointer touches or things like that. note next example of teleport script.
...
includes steamvr interact grab script. also got listener script which throws events.
includes steamvr interact touch script

put interactable object script on objects that you want to move
